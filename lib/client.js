window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		const css = [
			...
			".dss_introRow{flex-direction:row;gap:10px;align-items:center;justify-content:space-between;display:flex}",
			".dss_meta{font-size:12px;line-height:20px;color:var(--dsw-alias-label-tertiary);white-space:nowrap;font-variant-numeric:tabular-nums;flex:none}",
		...
		const C = {
			section: "dss_section", title: "dss_title", intro: "dss_intro", introRow: "dss_introRow", meta: "dss_meta", toolbar: "dss_toolbar",
		...
		function SkillsSettingsSection(props) {
			const { t, sessions, conversation } = props;
			...
			react.useEffect(() => {
				let cancelled = false;
				adminApi("/plugins/skills-admin/meta").then((data) => {
					if (cancelled) return;
					if (data && typeof data.name === "string") {
						setMeta({ name: data.name, version: typeof data.version === "string" ? data.version : "" });
					}
				}, () => { /* older host without /meta — leave the badge hidden */ });
				return () => { cancelled = true; };
			}, []);
			react.useEffect(() => {
			...
			// view content (list or detail); the whole section is wrapped in a
			// RenderGuard so a render crash degrades to an error row, never a blank panel
			let content;
			if (view === "detail" && detail !== null) {
				content = h(SkillDetail, {
			...
			return h("section", {
				...
				children: [
					...
					h("div", {
						className: C.introRow,
						children: [
							h("p", { className: C.intro, children: t("intro") }),
							meta !== null ? h("span", { className: C.meta, children: meta.name + " | " + meta.version }) : null
						]
					}),
					h("div", {
						className: C.toolbar,
			...
			return h(RenderGuard, { label: t("renderError"), backLabel: t("back"), onBack: backToList, children: content });
		}
		//#endregion
		...
		function apply(ctx) {
			...
			ctx.slots.inject("settings.section", () => ctx.slots.register({
				...
				label: () => t("nav"),
				locale: NS,
				inject: () => ({ sessions, conversation: ctx.get("conversation") })
